...
public function organizations(){
    return $this->belongsToMany(
        Organization::class, // 目标模型/表
        'user_had_organizations', // 中间表名
        'user_id', // 中间表对本表的外键
        'organization_id' // 中间表对目标模型/表的外键
    );
}
...
public function users()
{
    ...
    return $this->belongsToMany(User::class)->using(UserHadOrganization::class);; // using 使用自定义 中间模型 多对多
}
```
